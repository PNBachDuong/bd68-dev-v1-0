...
- Conditional activation:
  ...
  - `hierarchical-agent-memory`: default OFF. ON only for multi-session or long-horizon tasks and only when memory backend is stable (no repeated `5xx` or timeout in the same session). Keep OFF for one-shot or memory-degraded mode.
- Safety precedence:

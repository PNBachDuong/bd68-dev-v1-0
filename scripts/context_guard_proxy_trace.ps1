...
$clientToProxy = if ($null -ne $preChars) {
    "input -> proxy: ~$preTokens tokens (chars=$preChars)"
} else {
    "input -> proxy: ~$preTokens tokens"
}

$proxyToLlmgate = if ($null -ne $postChars) {
    "proxy -> llmgate: ~$postTokens tokens (chars=$postChars)"
} else {
    "proxy -> llmgate: ~$postTokens tokens"
}

$llmgateToProxy = if ($null -eq $modelIn -and $null -eq $modelOut) {
    "llmgate -> proxy: model usage N/A (need successful upstream usage fields)"
} else {
    "llmgate -> proxy: model_in=$modelIn model_out=$modelOut"
}

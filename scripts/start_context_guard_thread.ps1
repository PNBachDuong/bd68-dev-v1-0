...
$scriptPath = Join-Path $PSScriptRoot "context_guard_thread.js"
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Missing thread script: $scriptPath"
}
...
if ($Background.IsPresent) {
    $pidFile = Join-Path $PSScriptRoot "artifacts\context_guard_thread.pid"
    ...
    if (Test-Path -LiteralPath $pidFile) {
        ...
        if ($rawPid -match '^\d+$') {
            ...
            if ($existingAlive) {
                throw "Thread route already running with PID $existingPid. Stop it first."
            }
        }
    }
